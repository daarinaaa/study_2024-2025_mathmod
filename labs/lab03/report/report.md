...
function reg(u, p, t)
    x, y = u
    a, b, c, h = p
    dx = -a*x - b*y+sin(5.5*t)+1
    dy = -c*x -h*y+cos(2.5*t)+1
    return [dx, dy]
end

# начальные условия
u0 = [36800, 41700]
p = [0.35, 0.776, 0.519, 0.573]
tspan = (0,1)
...
model lab3
  ...
  parameter Real b = 0.776;
  parameter Real c = 0.519;
  parameter Real h = 0.573;
  parameter Real x0 = 3800;
  parameter Real y0 = 41700;
  Real x(start=x0);
  Real y(start=y0);
equation
  der(x) = -a*x - b*y+sin(5.5*time)+1;
  der(y) = -c*x -h*y+cos(2.5*time)+1;
end lab3;
```
...
function reg_part(u, p, t)
    ...
    a, b, c, h = p
    dx = -a*x - b*y+sin(2*t)
    dy = -c*x*y -h*y+cos(13*t)
    return [dx, dy]
end

# начальные условия
u0 = [36800, 41700]
p = [0.342, 0.615, 0.443, 0.4]
tspan = (0,1)\
...
В результате получаем слудющий график изменения численности армий (рис. [-@fig:005.5]):
...
```
model lab3_v2
  parameter Real a = 0.342;
  parameter Real b = 0.615;
  parameter Real c = 0.443;
  parameter Real h = 0.4;
  parameter Real x0 = 36800;
  parameter Real y0 = 41700;
  Real x(start=x0);
  Real y(start=y0);
equation
  der(x) = -a*x - b*y+sin(2*time);
  der(y) = -c*x*y -h*y+cos(13*time);
end lab5.5_v2;
```
